    ...
    def _get_map_data(self, map_id: int) -> Dict[Tuple[int, int], float]:
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                cursor = conn.execute("SELECT x, y, state, impassable_score FROM explored_locations WHERE map_id = ?", (map_id,))
                for row in cursor:
                    x, y, state, score = row
                    # Prioritize impassable_score, fallback to state
                    actual_score = score if score is not None else (1.0 if state == 2 else 0.0)
                    grid_costs[(x, y)] = actual_score
                
                # 2. Additional walls from replay_buffer (Fallback for hard stalls)
                cursor = conn.execute("SELECT coords, button FROM replay_buffer WHERE map_id = ? AND stuck = 1", (map_id,))
                for row in cursor:
                    try:
