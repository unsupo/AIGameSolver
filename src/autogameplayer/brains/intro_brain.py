...
class IntroMashBrain(Brain):
    ...
    def __init__(self, controller: Controller, optimizer: StrategyOptimizer = None, llm_client: LLMClientProtocol = None, **kwargs):
        ...
        self.reflector = ReflectionAgent(self.client, "nomic-embed-text", self.optimizer)
    ...
    async def act(self, observation: Observation, mcp_client=None) -> Action:
        ...
        if self.optimizer and observation.state.vision_vector:
            ...
            if macros:
                ...
                if best.get('reliability', 1.0) > 0.7:
                    print(f"⚡ Intro Shortcut: {best['description']} (Reliability: {best.get('reliability', 1.0):.2f})")
                    macro_actions = [Action(**step) for step in best['sequence']]
                    action = Action(macro=macro_actions, reasoning=f"Learned intro shortcut: {best['description']}")
        ...
        reasoning = f"Executing structured intro pattern step {step+1}: {p['button'].upper()}"
