...
class ReflectionAgent:
    ...
    async def analyze_session(self, session_id: str, ltm: Any, limit: int = 100):
        ...
        print(
            f"🕵️ Reflection: Running {scan_type} analysis on last {len(steps)} steps (Breakthrough Threshold: {breakthrough_threshold:.1f})"
        )
        ...
        SEMANTIC ABSTRACTION & HISTORY REWRITING:
        - UTILITY_DIALOGUE_SKIP: If you see more than 5 consecutive identical actions (like 'A' or 'B') while the game is in a dialogue/textbox state, categorize this sequence as 'UTILITY_DIALOGUE_SKIP'.
        - INTENT_ADVANCE_TEXT: Standardize all individual dialogue-clearing reasons to 'INTENT_ADVANCE_TEXT'.
        ...
        try:
            ...
            if not analysis:
                print(
                    f"⚠️ Reflection analysis failed: Could not extract JSON from response: {response[:100]}..."
                )
                return
            ...
            if ltm:
                await ltm.add_memory(
                    f"Session Reflection: {summary}",
                    {"type": "reflection", "session_id": session_id},
                )
            ...
            if self.knowledge:
                ...
                await self.knowledge.ingest_text(
                    knowledge_note, source=f"reflection_{session_id}"
                )
            ...
            for b in analysis.get("breakthroughs", []):
                ...
                start_step = next(
                    (s for s in steps if s["step_index"] == start_idx), None
                )
                if not start_step:
                    continue
                ...
                coords = (
                    json.loads(start_step["coords"])
                    if isinstance(start_step["coords"], str)
                    else start_step["coords"]
                )
                ...
                print(
                    f"✨ Gold Run Discovery: {b['reason']} (Efficiency: {efficiency:.2f}, Total Reward: {reward_sum:.1f}){f' -> {skill_name}' if skill_name else ''}"
                )
                ...
                v_end = (
                    np.frombuffer(end_step["vision_vector"], dtype=np.float32)
                    if end_step["vision_vector"]
                    else None
                )
                v_start = (
                    np.frombuffer(start_step["vision_vector"], dtype=np.float32)
                    if start_step["vision_vector"]
                    else None
                )
                ...
                if (
                    v_delta > 1.5 and map_id == end_map_id
                    ...
                    if self.knowledge:
                        await self.knowledge.ingest_text(
                            landmark_desc, source="visual_landmarks"
                        )
                # ------------------------------------------
            ...
            for trap_idx in analysis.get("trap_indices", []):
                ...
                if trap_vision:
                    print(
                        f"🗑️ Reflection identified TRAP at step {trap_idx}. Penalizing existing macro..."
                    )
    ...
    async def analyze_failure(
        ...
        try:
            ...
            if debug_info and "sequence" in debug_info:
                name = debug_info.get(
                    "proposed_skill_name", f"SKILL_DEBUG_{int(time.time())}"
                )
                print(
                    f"🛠️ Debug Reflection: {debug_info['analysis']} -> Creating {name}"
                )
