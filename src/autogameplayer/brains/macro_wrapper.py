...
class MacroAwareBrain(Brain):
    ...
    def _validate_macro_alignment(self, macro_actions: list, reasoning: str, observation: Observation) -> bool:
        """
        ...
        if any(k in goal for k in ["dialogue", "talk", "text", "advance"]):
            ...
            if any(b in ["up", "down", "left", "right"] for b in first_buttons) and "skip" not in macro_desc:
                # REJECT if we are already in dialogue (stuck in menu)
                if ctx.get("is_dialogue", False) or observation.state.has_dialogue_box:
                    print(f"⚖️ Voter: REJECTED movement shortcut '{reasoning}' - Goal is DIALOGUE and we are in a textbox.")
                    return False
        ...
        if any(k in goal for k in ["walk", "move", "go to", "exit", "enter"]):
            ...
            if all(b in ["a", "b"] for b in first_buttons) and not any(k in macro_desc for k in ["skip", "confirm", "open"]):
                # REJECT if in overworld (mashing A/B while walking is usually noise)
                if is_overworld:
                    print(f"⚖️ Voter: REJECTED button mash shortcut '{reasoning}' - Goal is MOVEMENT and we are in Overworld.")
                    return False
    ...
    async def act(self, observation: Observation, mcp_client=None) -> Action:
        ...
        try:
            ...
            with sqlite3.connect(str(self.optimizer.db_path), timeout=10) as conn:
                ...
                if row and row['reliability'] > 0.8:
                    ...
                    if self._validate_macro_alignment(macro_actions, row['description'], observation):
                        print(f"⚡ Hash Shortcut: {row['description']} (Reliability: {row['reliability']:.2f})")
        ...
        if not action and observation.state.vision_vector:
                ...
                try:
                    ...
                    with sqlite3.connect(str(self.optimizer.db_path), timeout=10) as conn:
                        ...
                        if row:
                            ...
                            print(f"🎬 Title Screen Detected! Triggering macro: {row['name'] or row['description']}")
                            self.optimizer.record_macro_usage_by_name(row['name'] or row['description'])
                            action = Action(
                                macro=macro_actions, 
            ...
            if not action:
                ...
                if macros:
                    best = macros[0]
                    intro_maps = [0]
                    if hasattr(self.inner, 'config') and self.inner.config and hasattr(self.inner.config, 'heuristics'):
                        intro_maps = self.inner.config.heuristics.intro_map_ids
                    
                    is_intro_phase = (map_id in intro_maps) or observation.state.is_intro_screen
                    ...
                    if best.get('reliability', 1.0) >= threshold:
                        ...
                        # --- FEATURE: Voter System ---
                        if self._validate_macro_alignment(macro_actions, best['description'], observation):
                            print(f"⚡ Macro Shortcut: {best['description']} (Reliability: {best.get('reliability', 1.0):.2f})")
            ...
            if action:
                ...
                if hasattr(self.inner, '_process_step_outcome'):
                    await self.inner._process_step_outcome(observation, mcp_client=mcp_client)
        ...
        if self.last_state_hash == current_hash:
            ...
            if self.stall_counter == 0 and len(self.pending_sequence) >= 2:
                ...
                if ltm:
                    import asyncio
                    asyncio.create_task(ltm.add_memory(msg, {"type": "discovery", "session_id": self.session_id}))
