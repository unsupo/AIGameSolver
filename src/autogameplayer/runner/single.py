...
def list_skills():
    ...
    try:
        with sqlite3.connect(str(db_path)) as conn:
            ...
            cursor = conn.execute("SELECT name, description, reliability, score, times_run, macro_json FROM skills ORDER BY (reliability * score) DESC")
            ...
            print(f"{'NAME':<20} | {'REL.':<5} | {'SCORE':<5} | {'RUNS':<4} | {'SEQUENCE SUMMARY'}")
            print("-" * 80)
            ...
            for r in rows:
                ...
                try:
                    ...
                    if isinstance(macro, list):
                        ...
                        for step in macro[:5]: # Show first 5 steps
                            ...
                            steps.append(f"{btn.upper()}" + (f"x{rep}" if rep > 1 else ""))
                ...
                print(f"{name:<20} | {reliability:<5} | {score:<5} | {runs:<4} | {seq_summary}")
            print("="*80 + "\n")
...
async def run():
    ...
    parser.add_argument("--verify", action="store_true", help="Run control verification before starting")
    parser.add_argument("--no-dashboard", action="store_true", help="Skip launching the Streamlit dashboard")
    parser.add_argument("--list-skills", action="store_true", help="List all learned skills from the database and exit")
    ...
    if not args.no_dashboard:
        ...
        ui_proc = subprocess.Popen(
            [sys.executable, "-m", "streamlit", "run", str(dashboard_path), "--server.headless", "true", "--server.port", "8501"],
            env=env, stdout=dashboard_log, stderr=subprocess.STDOUT
        )
