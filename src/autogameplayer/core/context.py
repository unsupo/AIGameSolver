...
class AgentContext:
    """
    State container passed to Solvers at every step.
    Contains exploration history, planner state, and memory contexts.
    """
    ...
    consecutive_stuck_steps: int = 0
    is_stuck: bool = False
    is_loop_detected: bool = False
    collision_history: list[Tuple[int, int]] = field(default_factory=list)
    visited_locations: Set[Tuple[int, int, int]] = field(
        default_factory=set
    )  # (map_id, x, y)

    # UI/Engine state
    is_dialogue: bool = False
    is_engine_locked: bool = False
    is_menu_open: bool = False

    # Macro Lock state
    active_macro: Optional[list] = field(default=None)

    # Goal state
    current_goal: Optional[str] = None
    goal_steps: list[str] = field(default_factory=list)
    ...
    def lock_to_macro(self, macro_sequence: list):
        """Locks the agent into executing a specific sequence of actions."""
        self.active_macro = list(macro_sequence)
...
class ProfileDrivenExtractor(RAMContextExtractor):
    ...
    def extract(self, emulator: BaseEmulator) -> Dict[str, Any]:
        ...
        try:
            ctx = {}
            if self.layout.map_id:
                ctx["map_id"] = emulator.read_memory(self.layout.map_id)
            if self.layout.x_pos:
                ctx["x"] = emulator.read_memory(self.layout.x_pos)
            if self.layout.y_pos:
                ctx["y"] = emulator.read_memory(self.layout.y_pos)
            ...
            menu_status = ctx.get("menu_status", 0) > 0

            if textbox_active or menu_id_active or menu_status:
            ...
            if (
                "money_byte_1" in ctx
                and "money_byte_2" in ctx
                and "money_byte_3" in ctx
            ):

                def bcd_to_int(bcd):
                    try:
                        return int(f"{bcd:02x}")
                    except ValueError:
                        return 0

                money = (
                    (bcd_to_int(ctx["money_byte_1"]) * 10000)
            ...
            if "battle_state" in ctx:
                if ctx["battle_state"] > 0:
                    if "enemy_hp_high" in ctx and "enemy_hp_low" in ctx:
                        ctx["enemy_hp"] = (ctx["enemy_hp_high"] * 256) + ctx[
                            "enemy_hp_low"
                        ]
                    if "player_active_hp_high" in ctx and "player_active_hp_low" in ctx:
                        ctx["player_active_hp"] = (
                            ctx["player_active_hp_high"] * 256
                        ) + ctx["player_active_hp_low"]

            if "party_count" in ctx and ctx["party_count"] > 0:
                if "slot_1_hp_high" in ctx and "slot_1_hp_low" in ctx:
                    ctx["slot_1_hp"] = (ctx["slot_1_hp_high"] * 256) + ctx[
                        "slot_1_hp_low"
                    ]
                if "slot_1_max_hp_high" in ctx and "slot_1_max_hp_low" in ctx:
                    ctx["slot_1_max_hp"] = (ctx["slot_1_max_hp_high"] * 256) + ctx[
                        "slot_1_max_hp_low"
                    ]
            ...
            symbolic = {
                ...
                "status": "EXPLORABLE (Overworld)"
                if ctx.get("is_overworld_active")
                else "LOCKED (Menu/Dialogue)",
            ...
            if "tile_under_player" in ctx:
                ...
                symbolic["current_tile"] = tile_map.get(
                    tile_val, f"GROUND (0x{tile_val:02x})"
                )
