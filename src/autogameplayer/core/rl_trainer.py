...
def train_rl(config_path: str, num_envs: int = 4, total_timesteps: int = 100000):
    ...
    try:
        ...
        print(
            "🧠 Model initialized. Beginning training loop. (Press Ctrl+C to stop safely)"
        )
...
def main():
    ...
    parser.add_argument(
        "--workers", type=int, default=4, help="Number of parallel emulators"
    )
    parser.add_argument(
        "--timesteps", type=int, default=100000, help="Total training steps"
    )
