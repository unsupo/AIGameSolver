...
class GeneticTrainer:
    ...
    def __init__(
        self, config: GameConfig, population_size: int = 20, num_workers: int = 4
    ):
        self.config = config
        ...
        # Ask the brain registry for the correct genome size for this brain
        # The 'evolution' brain is now our tripartite model refactor.
        genome_size = Registry.get_brain_genome_size("evolution", controller)
        print(f"🧬 Brain genome size: {genome_size} parameters.")

        self.pop_manager = PopulationManager(population_size, (genome_size, 1))
        self.orchestrator = Orchestrator(num_workers, self.rom_path, config=self.config)
        self.metrics_path = (
            settings.base_dir / "logs" / "metrics" / "evolution_stats.json"
        )
        self.history = []
    ...
    async def _check_initial_checkpoint(self):
        ...
        try:
            ...
            try:
                ...
                logging.debug(
                    f"No initial state found in slot {settings.bootstrap_slot}: {e}"
                )
    ...
    def train(self, generations: int = 50, steps_per_episode: int = 200):
        ...
        if not self.curriculum.checkpoints:
            print(
                "⚠️ Warning: No curriculum defined in profile. Training might be aimless."
            )
        ...
        for gen in range(1, generations + 1):
            ...
            results = self.orchestrator.evaluate_population(
                genomes, steps_per_episode=steps_per_episode, target_checkpoint=target
            )
            ...
            print(
                f"Summary: Success Rate: {success_rate:.1%}, Max Reward: {max_fitness:.2f}"
            )
            ...
            if target and success_rate >= self.success_threshold:
                print(
                    f"✨ MILESTONE REACHED! {success_count} individuals achieved '{target.name}'."
                )
                # Move the 'winning' state from milestone slot to bootstrap slot
                state_milestone = (
                    settings.base_dir
                    / f"state_slot_{settings.milestone_tmp_slot}.state"
                )
                state_bootstrap = (
                    settings.base_dir / f"state_slot_{settings.bootstrap_slot}.state"
                )
                if os.path.exists(state_milestone):
                    shutil.copy(state_milestone, state_bootstrap)
                    print(
                        f"💾 Advancing starting line to the new checkpoint (Slot {settings.bootstrap_slot})."
                    )
...
def main():
    ...
    parser.add_argument(
        "--config", type=str, required=True, help="Path to game config YAML"
    )
    parser.add_argument(
    ...
    trainer = GeneticTrainer(
        config=config, population_size=args.pop_size, num_workers=args.workers
    )
