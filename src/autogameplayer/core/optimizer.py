...
class StrategyOptimizer:
    ...
    def __init__(
        self, client: LLMClientProtocol = None, model: str = None, db_path: str = None
    ):
        self.client = client
        self.model = model
        if db_path is None:
    ...
    def _seed_global_skills(self):
        ...
        seeds = [
            ...
            {
                "name": "SKILL_NAMING",
                "description": "Bypass naming screen with default name (RED).",
                "macro": [
                    {"button": "a", "frames": 10},
                    {"button": "a", "frames": 10},
                    {"button": "a", "frames": 10},
                            ...
                            seeds.append(
                                {
                                    ...
                                    "description": data.get(
                                        "description", f"Loaded from {f_path.name}"
                                    ),
                                    "macro": data["sequence"],
                                }
                            )
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                for s in seeds:
                    ...
                    cursor = conn.execute(
                        "SELECT id FROM skills WHERE name = ?", (s["name"],)
                    )
    ...
    def save_macro(
        ...
        map_id: int,
        coords: tuple,
        description: str = "Discovered successful sequence.",
        score: float = 5.0,
        vision_hash: str = None,
        ocr_text: str = None,
        ...
        v_bytes = (
            np.array(vision_vector, dtype=np.float32).tobytes()
            if vision_vector
            else None
        )
        ...
        if vision_vector:
            ...
            try:
                with get_db_connection(self.db_path) as conn:
                    ...
                    cursor = conn.execute(
                        "SELECT id, vision_vector, score FROM skills WHERE map_id = ?",
                        (map_id,),
                    )
                    for row in cursor:
                        sid, v_blob, _old_score = (
                            row["id"],
                            row["vision_vector"],
                            row["score"],
                        )
    ...
    def promote_macro(self, vision_vector: List[float], outcome: float):
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    "SELECT id, map_id, vision_vector, reliability, times_run, times_succeeded, name FROM skills"
                )
                for row in cursor:
                    sid, v_blob, reliability, times_run, times_succeeded = (
                        row["id"],
                        row["vision_vector"],
                        row["reliability"],
                        row["times_run"],
                    ...
                    if is_match:
                        ...
                        if new_reliability < 0.2 and times_run > 3:
                            ...
                            if row["map_id"] != -1:
                                print(
                                    f"🗑️ Purging unreliable macro {sid} (Reliability: {new_reliability:.2f})"
                                )
                        ...
                        conn.commit()
                        print(
                            f"📈 Promoted Macro {sid} ({row['name'] or 'Unnamed'}): Reliability {new_reliability:.2f} (Run {times_run + 1})"
                        )
                        break
        except Exception as e:
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                ...
                for name in sub_skill_names:
                    cursor = conn.execute(
                        "SELECT macro_json, is_hierarchical FROM skills WHERE name = ? OR description = ? LIMIT 1",
                        (name, name),
                    )
                    row = cursor.fetchone()
                    if row:
                        if row["is_hierarchical"]:
                            flat_sequence.extend(
                                self._resolve_hierarchical_macro(row["macro_json"])
                            )
                        else:
    ...
    def get_relevant_macros(
        ...
        if threshold is None:
            threshold = settings.cosine_threshold_vision
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                # 1. Query by map context or global (-1)
                cursor = conn.execute(
                ...
                for row in cursor:
                    ...
                    # Case A: Visual Match (Primary - Agnostic)
                    ...
                    if (
                        not is_match
                        and vision_hash
                        and row["vision_hash"] == vision_hash
                    ):
                    ...
                    if is_match:
                        ...
                        if row["is_hierarchical"]:
                            sequence = self._resolve_hierarchical_macro(
                                row["macro_json"]
                            )
                        else:
    ...
    def get_best_macro_for_context(
        self,
        vision_vector: List[float],
        map_id: int,
        vision_hash: str = None,
        ...
        return self.get_relevant_macros(
            vision_vector, vision_hash=vision_hash, map_id=map_id
        )[:top_k]

    def get_best_macro(
        self, vision_vector: List[float], threshold: float = 0.80
    ...
    def penalize_macro(self, vision_vector: List[float]):
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    "SELECT id, vision_vector, reliability, times_run FROM skills"
                )
                for row in cursor:
                    sid, v_blob, reliability, times_run = (
                        row["id"],
                        row["vision_vector"],
                        row["reliability"],
                        row["times_run"],
                    ...
                    if v_blob:
                        ...
                        if sim > 0.80:
                            ...
                            if new_reliability < 0.2 and times_run > 3:
                                print(
                                    f"🗑️ Purging failed macro {sid} (Reliability dropped too low)."
                                )
                                conn.execute("DELETE FROM skills WHERE id = ?", (sid,))
                            else:
                                conn.execute(
    ...
    def penalize_macro_by_name(self, name: str):
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    "SELECT id, reliability FROM skills WHERE name = ?", (name,)
                )
                row = cursor.fetchone()
                if row:
                    new_reliability = (
                        row["reliability"] * 0.8
                    )  # Slightly harsher for explicit name penalty
                    conn.execute(
                        "UPDATE skills SET reliability = ? WHERE id = ?",
                        (new_reliability, row["id"]),
                    )
                    conn.commit()
                    print(
                        f"📉 Penalized Skill by Name: {name} -> New Reliability: {new_reliability:.2f}"
                    )
        except Exception:
            pass
    ...
    @database_write("db_path")
    def evolve_macro(
        self, vision_vector: List[float], new_sequence: List[Dict[str, Any]]
    ):
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    "SELECT id, vision_vector, macro_json FROM skills"
                )
                for row in cursor:
                    sid, v_blob, old_macro_json = (
                        row["id"],
                        row["vision_vector"],
                        row["macro_json"],
                    )
                        ...
                        if sim > 0.98:
                            try:
                                ...
                                if len(new_sequence) < len(old_seq):
                                    print(
                                        f"📈 Evolving macro {sid}: Compressed from {len(old_seq)} to {len(new_sequence)} steps."
                                    )
                                    conn.execute(
                                        "UPDATE skills SET macro_json = ? WHERE id = ?",
                                        (json.dumps(new_sequence), sid),
                                    )
                                    conn.commit()
    ...
    def _is_high_entropy(self, macro_json: str) -> bool:
        ...
        try:
            ...
            if not seq:
                return False
            unique_buttons = set(
                step.get("button", "none") for step in seq if step.get("button")
            )
    ...
    async def async_compress_skills(self):
        ...
        try:
            ...
            with get_db_connection(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    "SELECT id, map_id, macro_json, description FROM skills WHERE compressed = 0 LIMIT 5"
                )
            ...
            for skill in uncompressed:
                ...
                if not is_strategic:
                    ...
                    print(
                        f"🛠️ Categorized as Utility: {skill['description']} -> {new_name}"
                    )
                else:
                    ...
                    print(
                        f"✨ Distilled Strategic Skill: {skill['description']} -> {new_name}"
                    )
    ...
    @database_write("db_path")
    def consolidate_macros(self, threshold: float = 0.80):
        """Identifies near-duplicate macros using Centroid Clustering by sequence and semantic intent."""
        try:
            ...
            with get_db_connection(self.db_path) as conn:
                ...
                print(
                    f"🧹 Running Centroid Consolidation on {len(all_skills)} skills..."
                )

                # 1. Group by exact button sequence (The Centroid Key)
                clusters = defaultdict(list)
                for s in all_skills:
                    clusters[s["macro_json"]].append(s)

                to_delete = set()

                for seq_json, members in clusters.items():
                    if len(members) < 2:
                        continue

                    # 2. Within each sequence group, merge by semantic similarity (75% threshold)
                    # We pick the one with the highest reliability as the 'Centroid'
                    members.sort(
                        key=lambda x: x.get("reliability", 0.5) * x.get("score", 1.0),
                        reverse=True,
                    )
                    ...
                    for other in members[1:]:
                        desc_sim = SequenceMatcher(
                            None, centroid["description"], other["description"]
                        ).ratio()

                        # Aggressive standardizing for dialogue/text advancement
                        desc_lower = (
                            centroid["description"] + " " + other["description"]
                        ).lower()
                        is_dialogue = any(
                            k in desc_lower
                            for k in [
                        ...
                        if desc_sim > 0.75 or is_dialogue:
                            # Standardize Dialogue
                            if is_dialogue:
                                conn.execute(
                                    "UPDATE skills SET name = 'UTILITY_DIALOGUE_ADVANCE', description = 'Canonical text advancement' WHERE id = ?",
                                    (centroid["id"],),
                                )

                            # Merge reliability
                            new_rel = max(centroid["reliability"], other["reliability"])
                            conn.execute(
                                "UPDATE skills SET reliability = ? WHERE id = ?",
                                (new_rel, centroid["id"]),
                            )
                ...
                if to_delete:
                    print(
                        f"✅ Centroid Clustering: Merged {len(to_delete)} redundant skills."
                    )
            ...
            print(f"⚠️ Centroid consolidation failed: {e}")

    @database_write("db_path")
    def cleanup_database(self):
        """Prunes the long_term_memory to ensure high-quality generalization."""
        print("🧼 Dreaming: Running Utility Audit & Database Cleanup...")
        try:
            with get_db_connection(self.db_path) as conn:
                # 1. Delete 'Stagnation Traps' (Low reliability, high usage, no progress)
                # Prune macros that have reliability < 0.2 after multiple runs
                res = conn.execute(
                    "DELETE FROM skills WHERE reliability < 0.2 AND times_run > 3"
                )
                if res.rowcount > 0:
                    print(f"🗑️ Pruned {res.rowcount} low-reliability stagnation traps.")

                # 1b. Delete 'Useless Mined Skills' (High usage, zero success)
                res = conn.execute(
                    "DELETE FROM skills WHERE times_run > 10 AND times_succeeded == 0 AND reliability < 0.5"
                )
                if res.rowcount > 0:
                    print(f"🗑️ Pruned {res.rowcount} useless mined skills (0 success after 10+ runs).")

                # 2. Consolidate 'A' Spam (Semantic Generalization)
                # Standardize common dialogue advancement sequences
                res = conn.execute("""
                    UPDATE skills 
                    SET name = 'UTILITY_DIALOGUE_ADVANCE', 
                        description = 'Standardized text advancement sequence (Mined from A-spam)'
                    WHERE macro_json LIKE '%"button": "a"%' 
                      AND (ocr_text IS NOT NULL AND length(ocr_text) > 0)
                      AND reliability > 0.5
                """)
                if res.rowcount > 0:
                    print(
                        f"📖 Standardized {res.rowcount} dialogue skills to UTILITY_DIALOGUE_ADVANCE."
                    )

                # 3. Hash De-duplication (Collapse functional states)
                # If multiple state hashes share (map_id, x, y) but different OCR (like scrolling text)
                # we keep only the most reliable skills for that physical tile.
                # (Handled by enhance consolidate_macros logic above)

                # 4. Prune old 'Explored Locations' to keep pathfinding fast
                # We only keep the most relevant/recent world map data
                conn.execute(
                    "DELETE FROM explored_locations WHERE last_seen < ?",
                    (time.time() - (7 * 24 * 3600),),
                )
            ...
            self.consolidate_macros(threshold=0.80)
    ...
    def sync_to_yaml(self):
        ...
        try:
            ...
            with get_db_connection(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    "SELECT name, description, macro_json, score, reliability FROM skills ORDER BY map_id, score DESC"
                )
                skills = []
                for row in cursor:
                    skills.append(
    ...
    def mine_sequences(self, top_k: int = 5):
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                ...
                cursor = conn.execute(
                    "SELECT * FROM replay_buffer ORDER BY session_id, step_index"
                )
                ...
                for sid, steps in episodes.items():
                    for i in range(len(steps)):
                        for k in range(2, 6):
                            ...
                            sub = steps[i : i + k]
                            if any(s["map_id"] != sub[0]["map_id"] for s in sub):
                                continue
                ...
                for btns, occs in sequences.items():
                    ...
                    if score > 5.0:
                        scored.append({"btns": btns, "score": score, "occ": occs[0]})
                ...
                for item in scored[:top_k]:
                    ...
                    v_vec = (
                        np.frombuffer(
                            start_step["vision_vector"], dtype=np.float32
                        ).tolist()
                        if start_step["vision_vector"]
                        else None
                    )
                ...
                if promoted_count > 0:
                    print(
                        f"✅ Successfully mined and promoted {promoted_count} sequences."
                    )
    ...
    def _evolve_macro(
        self,
        parent_a: Optional[Dict[str, Any]],
        parent_b: Optional[Dict[str, Any]] = None,
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            seq_a = (
                json.loads(parent_a["macro_json"])
                if isinstance(parent_a["macro_json"], str)
                else parent_a["macro_json"]
            )
            if not seq_a:
                return None
            ...
            if parent_b:
                seq_b = (
                    json.loads(parent_b["macro_json"])
                    if isinstance(parent_b["macro_json"], str)
                    else parent_b["macro_json"]
                )
                if not seq_b:
    ...
    def evolve_population(self, top_k: int = 5, variants_per_macro: int = 2):
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                ...
                print(
                    f"✅ Evolution complete: Generated {len(new_variants)} new variants."
                )
                self.consolidate_macros()
    ...
    def _mutate_macro(self, sequence: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if mutation_type == "insert" and len(res) < 10:
            ...
            idx = random.randint(0, len(res) - 1)
            res[idx]["button"] = random.choice(
                ["a", "b", "start", "up", "down", "left", "right"]
            )

        return res

    def _crossover_macros(
    ...
    def compose_hierarchical_skill(
        self, name: str, sub_skill_names: List[str], description: str = ""
    ):
        """Manually composes a new hierarchical skill from existing ones."""
        print(
            f"🧩 Composing hierarchical skill: {name} ({len(sub_skill_names)} sub-skills)"
        )
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                ...
                if row:
                    v_vec = (
                        np.frombuffer(row["vision_vector"], dtype=np.float32).tolist()
        ...
        if v_vec:
            ...
            try:
                with get_db_connection(self.db_path) as conn:
                    conn.execute(
                        "UPDATE skills SET name = ? WHERE vision_hash = ? AND is_hierarchical = 1",
                        (name, v_hash),
                    )
                    conn.commit()
            except Exception:
    ...
    def resolve_skill(self, name: str) -> List[Dict[str, Any]]:
        ...
        try:
            with get_db_connection(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    "SELECT macro_json, is_hierarchical FROM skills WHERE name = ? LIMIT 1",
                    (name,),
                )
                row = cursor.fetchone()
                if row:
            ...
            if len(items) >= 3:
                for item in items:
                    ...
                    description = meta.get(
                        "description", f"Map {map_id} Success Sequence"
                    )
            ...
            for step in seq:
                ...
                if btn == current_btn:
                    ...
                    if current_btn:
                        parts.append(
                            f"Press {current_btn}"
                            + (f" {current_count} times" if current_count > 1 else "")
                        )
                    current_btn = btn
                    current_count = 1
