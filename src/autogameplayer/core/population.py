...
class PopulationManager:
    ...
    def evolve(self, fitness_scores: List[float], mutation_rate: float = 0.1):
        ...
        if not np.any(valid_mask):
            print("⚠️ WARNING: Entire generation failed! Resetting to random population.")
            self.population = [np.random.randn(self.genome_size) for _ in range(self.population_size)]
            return self.population
