...
class GameRunner:
    ...
    async def _run_branching_evaluation(
        self, map_id: int, num_branches: int = 4, steps: int = 50
    ):
        """
        Optimal Route Search: Saves state, runs parallel workers, and loads the best branch.
        """
        print(
            f"🌿 Branching Evaluation: Exploring {num_branches} parallel paths for {steps} steps..."
        )

        # 1. Save "Base" state for workers to load
        await self.env.client.call_tool(
            "manage_checkpoint", {"action": "save", "slot": 99}
        )
        ...
        # Safely unwrap the brain if it's using MacroAwareBrain
        target_brain = self.brain
        if hasattr(self.brain, "inner"):
            target_brain = self.brain.inner

        # Extract rom_path from the brain's config
        rom_path = (
            target_brain.config.rom
            if hasattr(target_brain, "config") and target_brain.config
            else ""
        )
        ...
        if best_idx != -1:
            print(
                f"🏆 Branch {best_idx} won with reward {best_reward:.2f}. Merging into main timeline."
            )
    ...
    async def run(self, steps: int = 100) -> float:
        ...
        if (
            hasattr(target_brain, "config")
            and target_brain.config
            and hasattr(target_brain.config, "heuristics")
        ):
        ...
        try:
            for i in range(1, steps + 1):
                # 1. Plan Phase
                await self.brain.plan(obs, mcp_client=self.env.client)
                ...
                if (
                    ...
                    # 2. Bootstrap Save
                    print(
                        f"💾 Saving Best-So-Far Bootstrap to Slot {settings.bootstrap_slot}..."
                    )
                    await self.env.client.call_tool(
                        "manage_checkpoint",
                    ...
                    if (
                        not i >= steps
                    ):  # Only if session has significant remaining steps
        ...
        if hasattr(target_brain, "reflector") and target_brain.reflector:
            print(
                f"🌙 Session End: AI is 'Dreaming' (Reflecting on session {session_id})..."
            )
            try:
                # We need access to LTM, which target_brain usually has
                if hasattr(target_brain, "long_term_memory"):
                    await target_brain.reflector.analyze_session(
                        session_id, target_brain.long_term_memory
                    )
