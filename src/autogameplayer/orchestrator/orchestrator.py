...
from autogameplayer.core.config_loader import discover_roms
...
class Orchestrator:
    ...
    def evaluate_population(self, genomes: List[np.ndarray], steps_per_episode: int = 100, target_checkpoint=None, initial_slot: int = 0) -> List[dict]:
        ...
        print(f"Starting evaluation of {population_size} individuals using {self.num_workers} workers...")
        results = []
            ...
            try:
                ...
                for i, future in enumerate(concurrent.futures.as_completed(futures)):
                    try:
                        res = future.result()
                        results.append(res)
                        print(f"Individual {i+1}/{population_size} evaluated. Reward: {res['reward']:.2f}, Goal Reached: {res['reached_target']}")
...
def main():
    ...
    dummy_genomes = [np.random.randn(384 * 8) for _ in range(8)]
