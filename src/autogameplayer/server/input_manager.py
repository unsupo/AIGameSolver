...
class InputManager:
    ...
    def queue_input(self, button: str, duration: int = 10, reasoning: str = "", repeat: int = 1, macro: list = None):
    ...
    def _execute_button_press(self, button, duration, reasoning, repeat):
        ...
        self.last_reasoning = reasoning if (reasoning and str(reasoning).strip()) else "Executing action sequence..."

        for _ in range(repeat):
            self._press_single_button(button, duration)
            if repeat > 1: time.sleep(frames_to_seconds(1))
    ...
    def _press_single_button(self, button, frames):
        ...
        if self.emulator.has_release:
            with self.lock:
                try:
                    self.recording.record_event(self.get_tick_count(), "keyup", key=button)
