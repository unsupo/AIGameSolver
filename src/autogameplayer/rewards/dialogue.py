...
class DialogueDiscoveryReward(RewardFunction):
    ...
    def __init__(self, reward_value: float = 5.0, category: str = "intrinsic", **kwargs):
        super().__init__(category=category)
        self.seen_text_hashes = set()
        self.reward_value = reward_value

    def reset(self) -> None:
        self.seen_text_hashes.clear()

    async def compute(self, prev_obs: Observation, obs: Observation) -> float:
        ocr_text = ""  # (obs.state.ocr_text or "").strip() # ocr is unreliable let's just not use it at all
        ...
        if text_hash not in self.seen_text_hashes:
            ...
            print(
                f"📖 New Dialogue Discovered: '{ocr_text[:30]}...' (+{self.reward_value})"
            )
