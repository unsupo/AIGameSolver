...
async def llm_coach_session(rom_path: str, model_path: str):
    ...
    env_vars["PYTHONPATH"] = (
        str(settings.base_dir / "src") + os.pathsep + env_vars.get("PYTHONPATH", "")
    )
    ...
    try:
        ...
        while True:
            ...
            if rl_model and stagnation_counter < 10:
                ...
                action_idx, lstm_states = rl_model.predict(
                    vec,
                    state=lstm_states,
            ...
            if reward <= 0:
                ...
                if stagnation_counter >= 10:
                    print(
                        "✅ LLM Coach got us unstuck! Saving new curriculum checkpoint."
                    )
                    await client.call_tool(
                        "manage_checkpoint",
        ...
        terminate_process(server_proc, "LLM Coach Server")
