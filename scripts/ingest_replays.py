...
async def summarize_sequence(steps):
    ...
    for i, s in enumerate(steps):
        ...
        ocr = s.get("ocr", "")
        seq_text.append(f"Step {i}: Map {ctx.get('map_id')} @ ({ctx.get('x')},{ctx.get('y')}) | Pressed {action.upper()} | OCR: '{ocr}'")
        ...
        try:
            ...
            for i, step in enumerate(data):
                ...
                if reward > 5.0: # Significant reward threshold
                    print(f"✨ Found reward spike ({reward}) in {file.name} at step {i}")
                    ...
                    if hint:
                        ...
                        await ltm.add_memory(desc, {"map_id": map_id, "type": "experience_summary"})
