...
async def run_dream_cycle():
    ...
    failed_attempts = [
        ...
        if "CRITICAL WARNING" in m["text"] or "FAILED" in m["text"]
    ]

    print(
        f"✅ Pruning complete. Reduced {initial_count} memories to {len(ltm.memories)} unique insights."
    )
    ...
    if len(failed_attempts) > 5:
        ...
        try:
            ...
            await ltm.add_memory(
                new_rule, {"type": "rule", "source": "dream_reflection"}
            )
