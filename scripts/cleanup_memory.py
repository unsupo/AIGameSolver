...
def cleanup_memory():
    ...
    for i, mem in enumerate(data["memories"]):
        ...
        if text.startswith(
            'CRITICAL RULE FOR MAP 38: The AI must not repeatedly execute the action "A"'
        ):
            continue
        # Filter out corrupted thoughts about mashing A at (3,6)
        if (
            "(3, 6)" in text
            or "maximize reward collection" in text
            or "Focus on repeatedly pressing 'A'" in text
        ):
            continue
