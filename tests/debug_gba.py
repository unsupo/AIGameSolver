...
def debug_boot():
    ...
    configs = [
        ...
        {"name": "Skip BIOS (False) + Downloaded BIOS", "skip_bios": False, "bios": bios}
    ]
    
    for cfg in configs:
        print(f"\nTesting: {cfg['name']}")
        try:
            gba = PyBoyAdvance(gamepak=rom, bios=cfg['bios'], skip_bios=cfg['skip_bios'])
            ...
            for i in range(1000):
                ...
                if i % 200 == 0:
                    ...
                    if non_zero > 0:
                        ...
                        Image.fromarray(rgb888).save(f"debug_{cfg['name'].replace(' ', '_')}.png")
                        break
            else:
